...
def mark_your_attendance():
    ...
    while True:
        ...
        result_count = cursor.execute('SELECT * FROM user_company_information WHERE username = %s',(name))
        print('SELECT * FROM user_company_information WHERE username = %s',(name))
        res = cursor.fetchone()
        print(res)
        userinfo=''
        if result_count > 0:
